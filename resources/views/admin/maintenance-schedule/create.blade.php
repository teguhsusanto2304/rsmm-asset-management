...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white rounded-xl border border-[#dbe1e6] p-4 sm:p-6">
        <h1 class="text-2xl font-black text-[#111518] mb-6">Buat Jadwal Pemeliharaan Baru</h1>

        <form method="POST" action="{{ route('maintenance-schedule.store') }}" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="name" required class="w-full form-input rounded-lg @error('name') border-red-500 @enderror" 
                       placeholder="contoh: Pemeriksaan Berkala Bulanan" value="{{ old('name') }}">
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full form-input rounded-lg" placeholder="Deskripsi detail jadwal pemeliharaan...">{{ old('description') }}</textarea>
            </div>

            {{-- Maintenance Notes --}}
            <div>
                <label class="block text-sm font-bold mb-2">Catatan Pemeliharaan</label>
                <textarea name="maintenance_notes" rows="3" class="w-full form-input rounded-lg" placeholder="Instruksi khusus untuk teknisi...">{{ old('maintenance_notes') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="status" required class="w-full form-input rounded-lg @error('status') border-red-500 @enderror">
                        ...
                        <option value="paused" {{ old('status') == 'paused' ? 'selected' : '' }}>Ditunda</option>
                    </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="estimated_cost" required min="0" step="1000" class="w-full form-input rounded-lg @error('estimated_cost') border-red-500 @enderror"
                           placeholder="100000" value="{{ old('estimated_cost', 0) }}">
                ...
                <div>
                    ...
                    <input type="number" name="estimated_hours" required min="1" max="100" class="w-full form-input rounded-lg @error('estimated_hours') border-red-500 @enderror"
                           placeholder="1" value="{{ old('estimated_hours', 1) }}">
